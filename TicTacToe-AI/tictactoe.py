...
"""
#generated by copilot automatically lmao??
def strategy_minimax(grid: State, player: Player) -> Action:
    
    #Renvoie le coup optimal pour le joueur donné en utilisant l'algorithme Minimax.
    
    def minimax(grid: State, player: Player) -> tuple[Score, Action]:
        if final(grid):
            return score(grid), None
        ...
        for action in legals(grid):
            new_grid = play(grid, player, action)
            current_score, _ = minimax(new_grid, O if player == X else X)
    ...
    _, action = minimax(grid, player)
    return action
def strategy_alpha_beta(grid: State, player: Player) -> Action:
    
    #Renvoie le coup optimal pour le joueur donné en utilisant l'algorithme Alpha-Beta.
    
    def alpha_beta(grid: State, player: Player, alpha: Score, beta: Score) -> tuple[Score, Action]:
        if final(grid):
            return score(grid), None
        ...
        for action in legals(grid):
            new_grid = play(grid, player, action)
            current_score, _ = alpha_beta(new_grid, O if player == X else X, alpha, beta)
    ...
    _, action = alpha_beta(grid, player, -float('inf'), float('inf'))
    return action
"""